...
const Inventory = () => {
  ...
  // FUNGSI FILTER: Menyaring barang berdasarkan nama atau kode secara real-time
  ...
  return (
    <div className="p-4">
      ...
      <div style={{ display: 'none' }}>
        {items.map(item => (
          <QRCodeCanvas id={`qr-source-${item.id}`} key={item.id} value={item.kode_barang} size={128} />
        ))}
      </div>

      <Table hover responsive className="bg-white shadow-sm rounded border">
        <thead className="table-light">
          <tr><th>Kode</th><th>Nama Barang</th><th>Kategori</th><th>Status</th><th>Aksi</th></tr>
        </thead>
        <tbody>
          {filteredItems.length > 0 ? (
            filteredItems.map(item => (
              <tr key={item.id}>
                ...
                <td>{item.kategori}</td>
                <td>
                  <Badge bg={item.status === 'Tersedia' ? 'success' : item.status === 'Dipinjam' ? 'warning' : 'danger'}>
                    {item.status}
                  </Badge>
                </td>
                <td>
                  <Button variant="link" className="text-dark me-1" title="Cetak Label" onClick={() => handlePrint(item)}>
                    <Printer size={18}/>
                  </Button>
                  <Button variant="link" className="text-primary me-1" onClick={() => handleEdit(item)}>
                    <Edit size={18}/>
                  </Button>
                  <Button variant="link" className="text-danger" onClick={() => deleteItem(item.id)}>
                    <Trash2 size={18}/>
                  </Button>
                </td>
              </tr>
      ...
      <Modal show={showEdit} onHide={() => setShowEdit(false)}>
        <Modal.Header closeButton><Modal.Title>Ubah Status Barang</Modal.Title></Modal.Header>
        <Form onSubmit={handleUpdate}>
          ...
          <Modal.Footer><Button type="submit" variant="primary">Simpan Perubahan</Button></Modal.Footer>
        </Form>
      </Modal>
    </div>
  );
